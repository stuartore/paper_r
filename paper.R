...
# 分组
bio_data <- bio_data %>%
  mutate(`My Group` = case_when(
    `AHI_index` > 15 & `if_ph` == 1 ~ "PH + OSA（+）",
    `AHI_index` <= 15 & `if_ph` == 1 ~ "PH + OSA（-）",
    `AHI_index` > 15 & `if_pa` == 1 ~ "PA + OSA（+）",
    `AHI_index` <= 15 & `if_pa` == 1 ~ "PA + OSA（-）",
    TRUE ~ NA_character_ # 为其他情况指定默认值，例如NA
  ))
...
bio_data$if_ph <- cut(bio_data$if_ph, 
  breaks = c(-Inf, 0.5, Inf),
  labels = c("Diagnosed PH", "Not Diagnosed PH"),
  include.lowest = TRUE
)

cat("\n", names(data))

print(descrTable(`My Group` ~ . , data = bio_data))
